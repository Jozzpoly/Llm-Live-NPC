...
Return one bounded JSON envelope with exactly two fields: originReasonId and proposal. originReasonId must be the exact id of one entry in reasons that most directly caused this judgement; it is causal attribution, not durable evidence. proposal is one ResidentLifeIntentProposal. Its commitmentDecision normally decides whether the newly perceived pressure should become a continuing resident commitment. It does not replace or complete recovered matters by implication. The sole bounded lifecycle exception is release_standing, which may propose ending one exact open standing social commitment already named in life; local admission must still prove that exact release authority. No commitmentDecision moves the resident, binds a run, grants body focus, mutates World, creates a physical fact or proves an external outcome. A later local admission step will decide whether the proposal is still legal and how any accepted intent or standing release can be grounded from current resident state.
