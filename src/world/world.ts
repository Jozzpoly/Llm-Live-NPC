...
function sweepHorizontalMovement(
  ...
  for (const blocker of blockers) {
    ...
    // Resolve only a boundary crossed from outside. If an authored actor already
    // begins inside this expanded blocker, do not snap it opposite its command;
    // leaving or moving within that overlap remains possible.
    if (targetX > start.x) {
...
function sweepVerticalMovement(
  ...
  for (const blocker of blockers) {
    ...
    // As on X, collision is an entry constraint, not depenetration. Existing
    // authored overlap is permitted by R2 and must not create reverse snapping.
    if (targetY > startY) {
...
export class World {
  ...
  private moveActor(actor: ActorEntity, input: WorldInput, seconds: number): void {
    ...
    // Keep the historical world-bound clamp as an independent final guard. The
    // sweep should already resolve between bounded targets, but authored starts
    // are intentionally not topology-validated by R2.
