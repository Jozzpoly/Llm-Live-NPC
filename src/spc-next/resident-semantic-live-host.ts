...
export type ResidentSemanticLiveArrival = Readonly<
  | {
      ...
      status: "decision";
      decision: Readonly<{ semanticCourse: string }>;
    }
  | {
      version: 1;
      arrivalId: string;
...
export class ResidentSemanticLiveHost {
   ...
   */
  async requestMatter(matterId: string, signal?: AbortSignal): Promise<ResidentSemanticLiveArrival> {
  ...
  /** Backward-compatible name with deliberately changed two-phase semantics. */
  ...
  admit(arrival: ResidentSemanticLiveArrival, admissionTick: number): ResidentSemanticLiveAdmission {
    ...
    if (settlement.status === "applied") {
      this.recordAdmission(arrival, admissionTick, "applied", `semantic revision ${settlement.matter.semanticRevision}`);
    ...
    // A decision arrival was parsed against the same strict semantic-course shape
    // before registration. Any later invalid/local rejection is therefore an
    // internal authority inconsistency; fail closed and retire whatever exact
    // attempt still exists rather than silently mutating meaning.
